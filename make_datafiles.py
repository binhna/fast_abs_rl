...
all_val_urls = "/Users/binhna/Downloads/AIMESOFT/data/all_dev.txt"
all_test_urls = "/Users/binhna/Downloads/AIMESOFT/data/all_test.txt"

cnn_tokenized_stories_dir = "/Users/binhna/Downloads/AIMESOFT/data/livedoor_stories_tokenized"
finished_files_dir = "/Users/binhna/Downloads/AIMESOFT/data/finished_files"

# These are the number of .story files we expect there to be in cnn_stories_dir
# and dm_stories_dir
num_expected_cnn_stories = 88483
# num_expected_dm_stories = 219506

def mecab_tokenizer(sentence):
    sentence = tokenizer.parse(sentence)
    result = [i.split('\t')[0] for i in sentence.split('\n')[:-2]]
    return result
...
def tokenize_stories(stories_dir, tokenized_stories_dir):
    ...
    for s in stories:
        with open(os.path.join(stories_dir, s), 'r') as f:
            lines = f.readlines()
        lines = [line.strip() for line in lines]
        for i, line in enumerate(lines):
            if line != '@highlight':
                line = ' '.join(mecab_tokenizer(line))
            lines[i] = line
        with open(os.path.join(tokenized_stories_dir, s), 'w') as f:
            for line in lines:
                f.write(line+'\n')
                f.write('\n')
    ...
    print("MeCab tokenizer has finished.")
...
def check_num_stories(stories_dir, num_expected):
    num_stories = len([file for file in os.listdir(stories_dir) if 'story' in file])
